...
class BaoNet(nn.Module):
    ...
    def __init__(
        ...
        print(
            f"Activation function in tree dense output layers: {self.activation_dense}"
        )
    ...
    def forward(self, x):
        trees = prepare_trees(
            x, self.features, left_child, right_child, cuda=self.__cuda
        )
...
class Autoencoder(nn.Module):
    def __init__(self, io_dim):
        ...
        self.encoder = nn.Sequential(nn.Linear(io_dim, 512), nn.ReLU())
        self.decoder = nn.Sequential(nn.Linear(512, io_dim), nn.Sigmoid())
...
class NeoNet(nn.Module):
    ...
    def __init__(
        ...
        :param tree_units_dense: list of  units in last layers after convolutional layers.
        """
        ...
        self.__in_channels = in_channels
        self.__cuda = False
        self.in_cuda = in_cuda
        self.activation_tree = activation_tree
        self.activation_dense = activation_dense
        self.query_model = QueryDataModel(
            query_input_size, query_hidden_inputs, query_output
        )
        print(f"Activation function in treeConv layers: {self.activation_tree}")
        print(
            f"Activation function in tree dense output layers: {self.activation_dense}"
            ...
            if i == 0:
                ...
                layers.append(
                    BinaryTreeConvWithQData(
                        self.__in_channels, query_output, tree_units[i]
                    )
                )
            else:
                layers.append(BinaryTreeConv(tree_units[i - 1], unit))
    ...
    def forward(self, data):
        ...
        del query_data
        trees = prepare_trees(
            tree_data, self.features, left_child, right_child, cuda=self.__cuda
        )
